...
const EMAILJS_TEMPLATE_ID_ADMIN = 'template_ihj0qk8'; // Replace with your actual admin template ID
...
const generateQuoteDetailsText = (data: any): string => {
  ...
  if (pieces.length > 0) {
    ...
    pieces.forEach(([letter, piece]: [string, any]) => {
      details += `• Piece ${letter}: ${piece.length}mm x ${piece.width}mm x ${piece.thickness}mm\n`;
    });
  }
  
  if (data.sinkOption && data.sinkOption !== 'no-sink') {
    details += `\nSink: ${data.sinkOption.replace(/-/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}\n`;
  }
...
const calculateTotalAmount = (data: any, settings: any): string => {
  // This is a simplified calculation - you can import your pricing calculator here
  ...
  const totalAreaMm2 = pieces.reduce((total: number, piece: any) => {
  ...
  const pricePerSqft = settings.pricePerSqft || 150;
  const basePrice = totalSqm * pricePerSqft * 10.764; // Convert sqm to sqft
  const estimatedTotal = Math.round(basePrice * 1.5); // Rough estimate with addons
